...
class NoteAPITest {
    ...
    fun `adding a Note to a populated list adds to ArrayList`() {
        ...
        assertEquals(5, populatedNotes!!.getNumberOfNotes())
        assertTrue(populatedNotes!!.add(newNote))
        assertEquals(6, populatedNotes!!.getNumberOfNotes())
        assertEquals(newNote, populatedNotes!!.findNote(populatedNotes!!.getNumberOfNotes() - 1))
    }

    @Test
    fun `adding a Note to an empty list adds to ArrayList`() {
        val newNote = Note("Study Lambdas", 1, "College", false)
        assertEquals(0, emptyNotes!!.getNumberOfNotes())
        assertTrue(emptyNotes!!.add(newNote))
        assertEquals(1, emptyNotes!!.getNumberOfNotes())
        assertEquals(newNote, emptyNotes!!.findNote(emptyNotes!!.getNumberOfNotes() - 1))
    }
